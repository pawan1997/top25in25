...
export default function VinylRecord({ user, index }: VinylRecordProps) {
  ...
  return (
    <>
      ...
      >
        {/* Vinyl Disc */}
        <div className="vinyl-disc w-full aspect-square max-w-[200px] relative mx-auto">
          {/* Mesh Gradient Background with soft fade */}
          <div className="absolute inset-0" style={{
            maskImage: 'radial-gradient(circle, rgba(0,0,0,1) 0%, rgba(0,0,0,0.9) 60%, rgba(0,0,0,0) 100%)',
            WebkitMaskImage: 'radial-gradient(circle, rgba(0,0,0,1) 0%, rgba(0,0,0,0.9) 60%, rgba(0,0,0,0) 100%)'
          }}>
            <ShaderBackdrop userId={user.id} size={200} />
          </div>

          {/* Center User Photo */}
          <div className="absolute inset-[12%] rounded-full overflow-hidden z-10 shadow-xl">
            <img
              src={user.imageUrl}
              alt={user.name}
        ...
        {/* Info Below Vinyl */}
        <div className="vinyl-info text-center mt-4 px-1">
          <h3 className="font-editorial text-sm md:text-base text-white leading-tight">{user.name}</h3>
          <p className="text-[11px] text-white/40 mt-1 truncate">{user.subtitle}</p>
        </div>
      </div>
