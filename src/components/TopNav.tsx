...
export default function TopNav() {
  ...
  useEffect(() => {
    ...
    gsap.fromTo(categoryItemsRef.current.filter(Boolean),
      { y: 20, opacity: 0 },
      {
        y: 0,
        opacity: 1,
  ...
  const renderCategoryCard = (category: typeof platformCategories[0], index: number) => {
    ...
    <>
      ...
      <nav className="fixed top-0 left-0 right-0 z-50">
        ...
        <div className="relative max-w-[1400px] mx-auto px-6 md:px-12 lg:px-16">
          ...
          {/* Desktop & Mobile category selector */}
          <div className={`
          ...
          {isDropdownOpen && (
            <div
              ...
              style={{ opacity: 0 }}
            >
              <div className="relative border border-white/10 rounded-xl bg-gradient-to-b from-black/95 via-black/95 to-black backdrop-blur-xl shadow-2xl p-3 md:p-6 h-full md:h-auto overflow-y-auto max-h-[80vh]">
                {/* Top glow */}
                <div
                ...
                {/* Section Header */}
                <h3 className="text-xs uppercase tracking-[0.2em] text-white/40 font-space mb-3 px-1">
                ...
                <div className="grid grid-cols-1 sm:grid-cols-2 md:grid-cols-3 gap-2 md:gap-3" style={{ perspective: '1000px' }}>
                  {(activeGroup === 'platform' ? platformCategories : industryCategories).map((cat, idx) => renderCategoryCard(cat, idx))}
                </div>
              </div>
            </div>
          )}
